...
def insert_product_to_db(product_name):
    with connect_db.cursor() as cursor:
        brand_name = "PARIS"
        brand_number = f"SELECT brand_number FROM Brand_table where brand = ?"
        cursor.execute(brand_number, brand_name)

        brand_number = int(brand_number)
        query = (
            f"INSERT INTO Product_table (brand_number , product_name) VALUES (? , ?)"
        )
        cursor.execute(query, (brand_number, product_name))

        connect_db.commit()
        print("新增成功")


with open("ingred.json", "r") as json_file:
